...
class Environment(Node):
    ...
    def __init__(self):
        ...
        # Prefer environment variable DRL_AGENT_SRC_PATH, but fall back to
        # several heuristics: ament package share, package relative path, etc.
        ...
        # build candidate roots to search for drl_agent/config/
        candidates = []
        if drl_agent_src_path:
            # If user provided workspace src path, consider its drl_agent subfolder
            candidates.append(os.path.join(drl_agent_src_path, "drl_agent"))

        # Try to get installed package share directory (if package is installed)
        try:
            ...
            # ament not available or package not registered; ignore
            pass

        # Try relative to this file: ../.. should point to drl_agent package root
        this_pkg_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
        candidates.append(this_pkg_root)

        # Also try one more level up in case of double 'src/src' layout
        maybe_ws_src = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
        candidates.append(os.path.join(maybe_ws_src, "drl_agent"))

        # Normalize and dedupe candidates while keeping order
        seen = set()
        normalized = []
        for c in candidates:
        ...
        for root in normalized:
            cfg = os.path.join(root, "config", env_config_file_name)
            tried.append(cfg)
            if os.path.exists(cfg):
                env_config_file_path = cfg
                start_goal_pairs_file_path = os.path.join(root, "config", start_goal_pairs_file)
                break

        if env_config_file_path is None:
            self.get_logger().error(
                f"Unable to find config file: tried paths: {tried}"
            )
            sys.exit(-1)
